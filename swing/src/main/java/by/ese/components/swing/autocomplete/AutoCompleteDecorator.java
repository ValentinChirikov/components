...
import by.ese.components.swing.autocomplete.adaptor.*;
import by.ese.components.swing.autocomplete.converter.ObjectToStringConverter;
import by.ese.components.swing.autocomplete.workarounds.MacOSXPopupLocationFix;
 ...
 */
@SuppressWarnings({"nls", "serial", "WeakerAccess"})
public class AutoCompleteDecorator {
    ...
    static AutoCompleteDocument createAutoCompleteDocument(
            AutoCompleteAdaptor adaptor, boolean strictMatching,
    ...
    public static void decorate(JComboBox comboBox, ObjectToStringConverter stringConverter) {
        ...
        // configure the editor text component
        JTextComponent editorTextComponent = (JTextComponent) comboBox.getEditor().getEditorComponent();
        final AutoCompleteAdaptor adaptor = comboBox instanceof EComboBox ? new FilterableComboBoxAdaptor((EComboBox) comboBox) : new ComboBoxAdaptor(comboBox);

        boolean strictMatching = !(comboBox.isEditable() || (adaptor instanceof FilterableComboBoxAdaptor));

        // has to be editable
        comboBox.setEditable(true);

        final AutoCompleteDocument autoCompleteDocument = createAutoCompleteDocument(adaptor, strictMatching,
                stringConverter, editorTextComponent.getDocument());

        decorate(editorTextComponent, autoCompleteDocument, adaptor);

        editorTextComponent.addKeyListener(new AutoComplete.KeyAdapter(comboBox));

        //set before adding the listener for the editor
        comboBox.setEditor(new AutoCompleteComboBoxEditor(comboBox.getEditor(), autoCompleteDocument.stringConverter));
     ...
     * @param stringConverter the converter used to transform items to strings
     */
    private static void decorate(JList list, JTextComponent textComponent, ObjectToStringConverter stringConverter) {
        undecorate(list);

        AutoCompleteAdaptor adaptor = new ListAdaptor(list, textComponent, stringConverter);
     ...
     * @param stringConverter the converter used to transform items to strings
     */
    private static void decorate(JTextComponent textComponent, List<?> items, boolean strictMatching, ObjectToStringConverter stringConverter) {
        AutoCompleteAdaptor adaptor = new TextComponentAdaptor(textComponent, items);
     ...
     * @param autoCompleteDocument      the AutoCompleteDocument to be installed on the text component
     * @param adaptor       the AbstractAutoCompleteAdaptor to be used
     */
    static void decorate(JTextComponent textComponent, AutoCompleteDocument autoCompleteDocument, AutoCompleteAdaptor adaptor) {
        undecorate(textComponent);

        // install the autoCompleteDocument on the text component
        ...
        while (editorInputMap != null) {
            ...
            if (parent instanceof UIResource) {
                installMap(editorInputMap, autoCompleteDocument.isStrictMatching());
                break;
            }
    ...
    static class NonStrictBackspaceAction extends TextAction {
        ...
        public NonStrictBackspaceAction(Action backspace, Action selectionBackward, AutoCompleteAdaptor adaptor) {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            if (adaptor.listContainsSelectedItem() && !(adaptor instanceof FilterableComboBoxAdaptor)) {
